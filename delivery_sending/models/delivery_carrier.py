...
SENDINGG_STATUS_CODES = {
    "9101": ("LOCALIZADO"),
    "9110": ("PROCEDAN A LA ENTREGA"),
    "1102": ("FALTAN BULTOS"),
    "2101": ("AVERIA EN RUTA REPARTO"),
    "3101": ("MERCANCIA ROBADA"),
    "4102": ("NO ACEPTAN MERCANCIA"),
    "4103": ("NO ACEPTAN P.D/REEMBOLSOS"),
    "4105": ("DEVOLUCION P.O. CLIENTE"),
    "5101": ("DIRECCION INCORRECTA"),
    "5102": ("FALTAN DATOS ENTREGA"),
    "5103": ("CODIGO POSTAL INCORRECTO"),
    "5104": ("EMPRESA DESAPARECIDA"),
    "5105": ("DESCONOCIDO DESTINATARIO"),
    "5106": ("CAMBIO DE DOMICILIO"),
    "6101": ("AUSENTE"),
    "6102": ("AUSENTE POR SEGUNDA VEZ"),
    "6106": ("AUSENTE POR TERCERA VEZ"),
    "6107": ("AUSENTE REITERADO"),
    "6205": ("CERRADO EN HORAS DE COMERCIO"),
    "7101": ("FESTIVO"),
    "7202": ("ENTREGA CONCERTADA PARA :"),
    "7301": ("PASARAN A RECOGER"),
    "CADE": ("EXPEDICION REEXPEDIDA"),
    "CMAI": ("COMUNICACIÓN POR EMAIL"),
    "LEC2": ("SALIDA RUTAS NOCTURNAS TRANSITO A DESTINO"),
    "DENT": ("ENVIO ORDENADO PARA DEVOLVER"),
    "DGEN": ("ENVIO DE DEVOLUCION"),
    "ENTR": ("ENTREGADA"),
    "LEC1": ("LECTURA RECEPCION EN ORIGEN"),
    "LEC3": ("LECTURA DESCARGA EN DESTINO"),
    "REEN": ("EXP CREADA EN REENVIO"),
    "REPA": ("EN REPARTO"),
    "ALTA": ("ALTA DE EXPEDICIÓN EN EL SISTEMA")
}
...
class DeliveryCarrier(models.Model):
    ...
    sending_group_id = fields.Char(string="Group")
    sending_user = fields.Char(string="User")
    sending_booking_type = fields.Selection(
        selection=[
            ("land", "Land"),
            ("air", "Air"),
            ("ocean_fcl", "Ocean FCL"),
    ...
    sending_service_land = fields.Selection(
        string="Land service",
        help="Land shipping product options. Depending on your customer account, some "
        "services could not be available",
        selection=[
            ("CON", "DBsendingconcepts"),
            ("DIR", "DBsendingdirects"),
            ("LPA", "DBsendingparcel Logistics Parcel"),
            ("PAL", "DBsendingpallets"),
            ("PRI", "DBsendingprivpark"),
            ("auc0", "austroexpress PUNKT 10"),
            ("auc2", "austroexpress PUNKT 12"),
            ("auc8", "austroexpress PUNKT 8"),
            ("aucc", "austroexpress PUNKT 17"),
            ("auco", "austrocargo"),
            ("ecsp", "sendingsystem-plus"),
            ("ect1", "DB sendingspeed 10"),
            ("ect2", "DB sendingspeed 12"),
            ("sch2", "DB sendingtop 12"),
            ("schs", "DB sendingsystem international"),
            ("sysd", "DB sendingsystem domestic"),
            ("scht", "DB sendingtop"),
            ("schx", "DB sendingsystem fix"),
            ("ecpa", "DB sendingparcel"),
            ("ect8", "DB sendingspeed 8"),
            ("ectn", "DB sendingspeed"),
            ("40", "DB sendingsystem classic"),
            ("41", "DB sendingsystem speed"),
            ("42", "DB sendingsystem fixday"),
            ("43", "DBsending System"),
            ("44", "DBsending System Premium"),
            ("71", "DB sendingdirect"),
        ],
    )
    sending_service_air = fields.Selection(
        string="Air service",
        help="Air shipping product options. Depending on your customer account, some "
        "services could not be available",
        selection=[
            ("f", "DB sendingjetcargo first"),
            ("s", "DB sendingjetcargo special"),
            ("b", "DB sendingjetcargo business"),
            ("e", "DB sendingjetcargo economy"),
            ("eagd", "DB sendingjetexpress gold"),
            ("easv", "DB sendingjetexpress silver"),
        ],
    )
    sending_indoor_delivery = fields.Boolean(
        string="Indoor Delivery", help="Defines if indoor delivery is required",
    )
    sending_express = fields.Boolean(
        string="Express", help="Defines if shipment is express",
    )
    sending_food_related = fields.Boolean(
        string="Food Related", help="Defines if shipment is food related",
    )
    sending_heated_transport = fields.Boolean(
        string="Heated Transport",
        help="Defines if shipment is required heated transport",
    )
    sending_home_delivery = fields.Boolean(
        string="Home Delivery", help="Defines if shipment is required home delivery",
    )
    sending_own_pickup = fields.Boolean(
        string="Own Pickup", help="Defines if shipment is required own pickup",
    )
    sending_pharmaceuticals = fields.Boolean(
        string="Pharmaceuticals", help="Defines if shipment is pharmaceutical",
    )
    sending_measure_unit = fields.Selection(
        string="Measure Unit",
    ...
    sending_default_packaging_id = fields.Many2one(
        comodel_name="product.packaging",
        string="Default Package Type",
        domain=[("package_carrier_type", "=", "sending")],
        help="If not delivery package or the package doesn't have defined the packaging"
        "it will default to this type",
    )

    def _get_sending_credentials(self):
        """Access key is mandatory for every request while group and user are
        optional"""
        credentials = {
            "prod": self.prod_environment,
            "access_key": self.sending_access_key,
        }
        if self.sending_group_id:
            credentials["group_id"] = self.sending_group_id
        if self.sending_user:
            credentials["user"] = self.sending_user
        return credentials

    @api.model
    def _sending_log_request(self, sending_request, picking):
        ...
        sending_last_request = sending_last_response = False
        try:
            sending_last_request = etree.tostring(
                sending_request.history.last_sent["envelope"],
                encoding="UTF-8",
                pretty_print=True,
            )
            sending_last_response = etree.tostring(
                sending_request.history.last_received["envelope"],
                encoding="UTF-8",
                pretty_print=True,
            )
        ...
        self.log_xml(sending_last_request, "sending_request")
        self.log_xml(sending_last_response, "sending_response")

    def _prepare_sending_barcode(self):
        ...
        vals = {
            "barcodeRequest": self.sending_barcode_format,
        }
        if self.sending_barcode_mail:
            vals["barcodeRequestEmail"] = self.sending_barcode_mail
        if self.sending_barcode_format == "A6":
            return vals
        # This options only can be informed when the label format is A4
        vals.update(
            {
                "start_pos": self.sending_barcode_a4_start_pos,
                "separated": self.sending_barcode_a4_separated,
            }
        )
        return vals

    def _prepare_sending_address(
        self,
        partner,
    ...
    def _sending_shipping_address(self, picking):
        ...
        return [
            self._prepare_sending_address(shipper_address, "SHIPPER"),
            self._prepare_sending_address(consignee_address),
        ]

    def _sending_shipping_product(self):
        """Gets the proper shipping product according to the shipping type
        :returns string with shipping product code
        """
        type_mapping = {
            "air": self.sending_service_air,
            "land": self.sending_service_land,
            "ocean_fcl": "fcl",
            "ocean_lcl": "lcl",
        }
        return type_mapping[self.sending_booking_type]

    def _sending_metric_system(self):
        """
        :returns string with sending metric system (METRIC or IMPERIAL)
        """
    ...
    def _sending_pickup_dates(self, picking):
        """Convert picking dates for sending api. We're taking the whole delivery
    ...
    def _sending_shipping_information_package(self, picking, package):
        ...
        return {
            ...
            "packageType": (
                ...
                or self.sending_default_packaging_id.shipper_package_code
            ),
            "stackable": (
                package.packaging_id.sending_stackable
                or self.sending_default_packaging_id.sending_stackable
            ),
            "pieces": 1,
        }

    def _sending_shipping_information(self, picking):
        ...
        if picking.package_level_ids and picking.package_ids:
            return [
                self._sending_shipping_information_package(picking, package)
        ...
        return [
            {
                ...
                "packageType": self.sending_default_packaging_id.shipper_package_code,
                "stackable": self.sending_default_packaging_id.sending_stackable,
                "pieces": picking.number_of_packages,
            }
        ]

    def _sending_measures(self, picking):
        ...
        """
        if self.sending_measure_unit == "VOLUME":
            return {
    ...
    def _prepare_sending_shipping(self, picking):
        """Convert picking values for sending api
        :param picking record with picking to send
        ...
        vals = {}
        vals.update(self._prepare_sending_barcode())
        vals.update(
            {
                "address": self._sending_shipping_address(picking),
                "incoterm": (
                    picking.sale_id.incoterm.code or self.sending_incoterm_id.code
                ),
                # A maximum of 35 characters is supported
                "incotermLocation": picking.partner_id.display_name[:35],
                "productCode": self._sending_shipping_product(),
                "measurementType": self._sending_metric_system(),
                "grossWeight": round(picking.shipping_weight, 2),
                "shippingInformation": {
                    "shipmentPosition": self._sending_shipping_information(picking),
                    "grossWeight": round(picking.shipping_weight, 2),
                    "volume": round(picking.volume, 2) or 0.01,
                },
                "measureUnit": self.sending_measure_unit,
                # Customs Clearance not supported for now as it needs a full customs
                # implementation
                "customsClearance": False,
                # Defines a business scenario where the sending customer sends a
                # booking request in the name of his ordering party
                "neutralShipping": False,
                "pickupDates": self._sending_pickup_dates(picking),
                # Not supported for the moment
                "specialCargo": False,
                "specialCargoDescription": False,
                "serviceType": self.sending_service_type,
                "indoorDelivery": self.sending_indoor_delivery,
                "express": self.sending_express,
                "foodRelated": self.sending_food_related,
                "heatedTransport": self.sending_heated_transport,
                "homeDelivery": self.sending_home_delivery,
                "ownPickup": self.sending_own_pickup,
                "pharmaceuticals": self.sending_pharmaceuticals,
            }
        )
        vals.update(self._sending_measures(picking))
        return vals

    def sending_send_shipping(self, pickings):
        """Send booking request to sending
        ...
        """
        sending_request = sendingRequest(**self._get_sending_credentials())
        result = []
        for picking in pickings:
            vals = self._prepare_sending_shipping(picking)
            vals.update({"tracking_number": False, "exact_price": 0})
            try:
                response = sending_request._send_shipping(
                    vals, self.sending_booking_type
                )
            except Exception as e:
                raise (e)
            finally:
                self._sending_log_request(sending_request, picking)
            if not response:
                result.append(vals)
                continue
            ...
            body = _("sending Shipping barcode document")
            attachment = []
            if response.get("barcode"):
                attachment = [
                    (
                        "sending_label_{}.pdf".format(response.get("booking_id")),
    ...
    def sending_cancel_shipment(self, pickings):
        ...
        for picking in pickings.filtered("carrier_tracking_ref"):
            try:
                sending_request._cancel_shipment(picking.carrier_tracking_ref)
            except Exception as e:
                raise (e)
            finally:
                self._sending_log_request(sending_request, picking)
    ...
    def sending_get_label(self, reference):
        ...
        if not reference:
            return False
        sending_request = sendingRequest(**self._get_sending_credentials())
        format_vals = self.sending_barcode_format
        if format_vals == "A4":
            format_vals = {
                "start_pos": self.sending_barcode_a4_start_pos,
                "_value_1": self.sending_barcode_format,
            }
        label = sending_request._shipping_label([reference], format_vals)
        if not label:
            return False
        return label

    def sending_get_tracking_link(self, picking):
        """Provide tracking link for the customer"""
        return (
            "https://esending.dbsending.com/app/tracking-public/?refNumber=%s"
            % picking.carrier_tracking_ref
        )

    def _prepare_sending_tracking(self, picking):
        self.ensure_one()
        return {
            "reference": picking.carrier_tracking_ref,
            "reference_type": "cu",
            "booking_type": self.sending_booking_type,
        }

    def sending_tracking_state_update(self, picking):
        """Tracking state update"""
        self.ensure_one()
        if not picking.carrier_tracking_ref:
            return
        sending_request = sendingRequest(
            **self._get_sending_credentials(), service="tracking"
        )
        response = sending_request._get_tracking_states(
            **self._prepare_sending_tracking(picking)
        )
        if response.get("shipment"):
            ...
            last_event = sending_STATUS_CODES.get(info.LastEvent, ("",))
            picking.write(
                {
    ...
    def sending_rate_shipment(self, order):
        ...
        raise NotImplementedError(
            _(
                "sending API doesn't provide methods to compute delivery "
    ...
    @api.onchange("sending_booking_type")
    def onchange_sending_booking_type(self):
        ...
        support"""
        if self.sending_booking_type != "land":
            raise UserError(_("Only land shipping is currently supported"))

    @api.onchange("sending_measure_unit")
    def onchange_sending_measure_unit(self):
